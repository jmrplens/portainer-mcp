...
┌──────────────────────▼──────────────────────────┐
│                  MCP Server                      │
│  cmd/portainer-mcp-enhanced/mcp.go                        │
│  ┌─────────────────────────────────────────────┐ │
...
<Aside type="tip">
The `PortainerClient` interface in `server.go` composes 18 domain-specific sub-interfaces from `client_interfaces.go`. This is the central contract. Every handler depends on it, and the mock in `mocks_test.go` implements it for unit tests.
</Aside>
