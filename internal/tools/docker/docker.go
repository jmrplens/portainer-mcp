...
func narrative(operationID string) toolutil.ActionNarrative {
	...
	case "ContainerImageStatus":
		return toolutil.ActionNarrative{
			Description: "Reports whether a newer version of one container's image is available. The sibling docker.service_image_status action has been measured returning a stale cached answer for a resource that had already stopped existing unless refresh is passed; this action declares the identical refresh parameter and may cache the same way, but that has not itself been verified — pass refresh if the answer needs to reflect the container's current state. A nonexistent containerId does not itself produce an error here: it has been measured answering Status=\"skipped\" instead, indistinguishable from a container whose image check was genuinely skipped for policy reasons — confirm the container exists (for example via docker.container_gpus_inspect, which does 404 on an unknown id) before treating a \"skipped\" result as informative. containerId is Docker's own 64-character hexadecimal container ID — published here as a string even though the vendored specification incorrectly declares it an integer; see docs/api-divergences.md.",
		}
