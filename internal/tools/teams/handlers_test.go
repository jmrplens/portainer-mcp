...
// domain's five routes carries an operationId in it, so nothing here depends
// on internal/specnaming's synthetic-name table the way endpoint_groups does.
...
// teams.delete's narrative carries the one measured fact about this domain a
// model cannot get anywhere else — that deleting a team takes its
// memberships with it — and losing it costs nothing that any other check can
// see.
