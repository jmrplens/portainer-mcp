...
// consulted by TestUnit_RedactionGuards_RemoveEveryCredentialShapedField and
// TestUnit_RedactionGuards_HandlerRedactsCredentialShapedFields.
//
// Six entries were scaffolded, one per generated wrapper. The last three
// were added by hand and are the ones that matter most: EndpointList,
// EndpointCreate and EndpointUpdate all return an environment carrying an
// Azure key, an edge key and TLS certificate paths, and all three have a
// hand-written handler in handlers.go. cmd/gen_action_inputs refuses to
// generate a handler that does not call its wrapper, and
// cmd/audit_spec_drift statically checks that a declared handler calls it —
// but the generator is exactly the thing that refused these three, and
// audit_spec_drift's check is an AST scan that a handler calling its wrapper
// and discarding the result would still satisfy. Running the handler end to
// end against a populated response is what actually discriminates, which is
// why these three belong in this table and not only in the wrapper-only
// guard above it.
var redactionGuards = []struct {
	...
	{funcName: "redactEndpointAssociationDelete", operationID: "EndpointAssociationDelete", fn: redactEndpointAssociationDelete, handler: endpointAssociationDelete},
	{funcName: "redactEndpointInspect", operationID: "EndpointInspect", fn: redactEndpointInspect, handler: endpointInspect},
	{funcName: "redactEndpointMTLSAgentCertificateError", operationID: "EndpointMTLSAgentCertificateError", fn: redactEndpointMTLSAgentCertificateError, handler: endpointMTLSAgentCertificateError},
	{funcName: "redactEndpointMTLSCertificate", operationID: "EndpointMTLSCertificate", fn: redactEndpointMTLSCertificate, handler: endpointMTLSCertificate},
	{funcName: "redactEndpointRegistriesList", operationID: "EndpointRegistriesList", fn: redactEndpointRegistriesList, handler: endpointRegistriesList},
	{funcName: "redactEndpointSettingsUpdate", operationID: "EndpointSettingsUpdate", fn: redactEndpointSettingsUpdate, handler: endpointSettingsUpdate},
	{funcName: "redactEndpointList", operationID: "EndpointList", fn: redactEndpointList, handler: endpointList},
	{funcName: "redactEndpointCreate", operationID: "EndpointCreate", fn: redactEndpointCreate, handler: endpointCreate},
	{funcName: "redactEndpointUpdate", operationID: "EndpointUpdate", fn: redactEndpointUpdate, handler: endpointUpdate},
}
