...
func renderRedactionGuardFile(packageName, specPath string, guards []redactionGuard) ([]byte, error) {
	...
	for _, g := range guards {
		fmt.Fprintf(&buf, `
// TestGenerated_%s_RemovesEveryCredentialShapedField proves %s actually
// strips what it is required to strip. cmd/gen_action_inputs refuses to emit
// a handler for %s unless a function of this name exists; existing is not the
// same as working, and this is what tells the two apart.
func TestGenerated_%s_RemovesEveryCredentialShapedField(t *testing.T) {
	t.Parallel()
	...
	survived := toolutil.AssertRedacted(fn.Call([]reflect.Value{arg})[0].Interface(), %q)
	if len(survived) > 0 {
		t.Errorf("%s left %%d credential-shaped field(s) populated: %%v", len(survived), survived)
	}
}
`, g.FuncName, g.FuncName, g.OperationID, g.FuncName, g.FuncName, g.FuncName, g.OperationID, g.FuncName)
	}
